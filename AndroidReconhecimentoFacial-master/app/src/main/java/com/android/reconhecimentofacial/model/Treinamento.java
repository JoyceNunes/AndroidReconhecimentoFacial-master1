package com.android.reconhecimentofacial.model;
...
public class Treinamento {
    ...
    public static final String TAG = "Treinamento";
    public static final String treinar_FOLDER = "treinar_folder";
    public static final int IMG_SIZE = 160;
    public static final String CLASSIFICADOR_EIGEN_FACES = "eigenFacesClassifier.yml";
    public static final String CLASSIFICADOR_FISHER_FACES = "fisherFacesClassifier.yml";
    public static final String CLASSIFICADOR_LBPH = "lbphClassifier.yml";
    public static final String FILE_NAME_PATTERN = "person.%d.%d.jpg";
    public static final int FOTOS_TREINAR_QTD = 25;
    public static final double NIVEL_DE_ACEITACAO = 2450.0D;
    public static void reset(Context context) throws Exception {
        File photosFolder = new File(context.getFilesDir(), treinar_FOLDER);
        if(photosFolder.exists()) {
            FilenameFilter imageFilter = new FilenameFilter() {                @Override
    ...
    public static boolean istreinared(Context context) {
        try {
            File photosFolder = new File(context.getFilesDir(), treinar_FOLDER);
            if(photosFolder.exists()) {
                ...
                FilenameFilter treinarFilter = new FilenameFilter() {
                    @Override
                    public boolean accept(File dir, String name) {
                ...
                File[] treinar = photosFolder.listFiles(treinarFilter);
                return photos!= null && treinar!= null && photos.length == FOTOS_TREINAR_QTD && treinar.length > 0;
            } else {
                return false;
            }
        }catch (Exception e) {
    ...
    public static int qtdFotos(Context context) {
        File photosFolder = new File(context.getFilesDir(), treinar_FOLDER);
        if(photosFolder.exists()) {
            FilenameFilter imageFilter = new FilenameFilter() {
                @Override
    ...
    public static boolean treinar(Context context) throws Exception{
        File photosFolder = new File(context.getFilesDir(), treinar_FOLDER);
        ...
        File f = new File(photosFolder, CLASSIFICADOR_EIGEN_FACES);
        f.createNewFile();
        eigenfaces.save(f.getAbsolutePath());



//      Implementa os outros reconhecdores
//        fisherfaces.train(photos, labels);
//        f = new File(photosFolder, CLASSIFICADOR_FISHER_FACES);
//        f.createNewFile();
//        fisherfaces.save(f.getAbsolutePath());

//        lbph.train(photos, labels);
//        f = new File(photosFolder, CLASSIFICADOR_LBPH);
//        f.createNewFile();
//        lbph.save(f.getAbsolutePath());
    ...
    public static void tirarFoto(Context context, int personId, int photoNumber, Mat rgbaMat, opencv_objdetect.CascadeClassifier faceDetector) throws Exception{
        File folder = new File(context.getFilesDir(), treinar_FOLDER);
        if(folder.exists() && !folder.isDirectory())
            folder.delete();
        if(!folder.exists())
            folder.mkdirs();
        int qtyPhotos = FOTOS_TREINAR_QTD;
